const UI = {
    ...
    initEventListeners: function() {
        this.elements.filterContainer.addEventListener('click', e => { const btn = e.target.closest('button[data-filter]'); if (btn) this.showSubMenu(btn.dataset.filter); });
        this.elements.subMenuContainer.addEventListener('click', e => { const btn = e.target.closest('button[data-parent]'); if (btn) { const p = AppState.subFilterMap[btn.dataset.parent][btn.dataset.name]; this.findAndDisplayPlaces(p); } });
        ...
        this.elements.resultsList.addEventListener('click', e => { const btn = e.target.closest('button[data-place-id]'); if (btn) this.showPlaceDetails(btn.dataset.placeId); });
        this.elements.backToResultsButton.addEventListener('click', () => { this.elements.resultScreen.classList.add('hidden'); this.elements.resultsListScreen.classList.remove('hidden'); });
    ...
    findAndDisplayPlaces: async function(params) {
        ...
        } catch { this.updateStatus('Search failed.'); }
    },
    ...
    renderResultsList: function(results) {
        this.elements.resultsList.innerHTML = results.map(p => `
            <button class="action-card w-full text-left p-2 flex gap-3 items-center" data-place-id="${p.place_id}">
                <img class="w-16 h-16 rounded-md object-cover bg-navy-light" src="${p.photos ? p.photos[0].getUrl({maxWidth:200}) : ''}" />
                <div><h3>${p.name}</h3><p class="text-sm" style="color:var(--accent-gold)">★ ${p.rating}</p></div>
            </button>
        `).join('');
        this.elements.resultsListScreen.classList.remove('hidden');
    },

    showPlaceDetails: async function(placeId) {
        ...
        try {
            ...
            this.updateStatus('');
        } catch { this.updateStatus('Could not load details.'); }
    },

    renderDetailedCard: function(p) {
        ...
        this.elements.placeRating.textContent = `${p.rating?.toFixed(1) || 'N/A'} (${p.user_ratings_total || 0})`;
        ...
        this.elements.directionsLink.href = `https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(p.vicinity)}&destination_place_id=${p.place_id}`;
    ...
    renderSavedPlaces: function() {
        const places = AppState.savedPlaces;
        this.elements.savedList.innerHTML = Object.keys(places).length ? Object.entries(places).map(([id, p]) => `
    ...
    toggleSaveCurrentPlace: function() { AppState.toggleSavedPlace(AppState.currentPlace); this.updateSaveButton(); },
    updateSaveButton: function() { this.elements.savePlaceButton.textContent = AppState.savedPlaces[AppState.currentPlace?.place_id] ? '★' : '☆'; },
    renderStarsHTML: function(r) { let h = ''; r = Math.round(r || 0); for (let i = 1; i <= 5; i++) h += `<span class="star ${i <= r ? 'filled' : ''}">★</span>`; return h; },
    showSubMenu: function(filter) { /* ... same as before ... */ },
    hideSubMenu: function() { /* ... same as before ... */ },
    findRandomAdventure: function() { /* ... same as before ... */ },
    updateStatus: function(text) { this.elements.locationStatus.textContent = text; },
};
