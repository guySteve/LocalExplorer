...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Recreation.gov API request failed:', response.status, response.statusText);
      return {
        statusCode: response.status,
        headers,
        body: JSON.stringify({ error: `Recreation.gov API error: ${response.statusText}` })
      };
    }
